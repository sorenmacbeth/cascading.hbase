...
public class HBaseTuplesToMap extends BaseOperation<HBaseTuplesToMap.AggregatorWriterTuplesListContext> implements
        Aggregator<HBaseTuplesToMap.AggregatorWriterTuplesListContext>
{
	        ...
	        AggregatorCall<HBaseTuplesToMap.AggregatorWriterTuplesListContext> aggregatorCall)
	{
		...
		byte[] columnFieldBytes = Bytes.toBytes(aggregatorCall.getArguments().getString(columnField));
		byte[] valueFieldBytes = Bytes.toBytes(aggregatorCall.getArguments().getString(valueField));
	        ...
	        AggregatorCall<HBaseTuplesToMap.AggregatorWriterTuplesListContext> aggregatorCall)
	{
		...
		aggregatorCall.getOutputCollector().add(
		        new Tuple(aggregatorCall.getContext().key, aggregatorCall.getContext().keyValueMap));
